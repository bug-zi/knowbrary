...
import { buildFunFactPrompt } from '~/utils/ai-prompts'
...
export default defineEventHandler(async (event) => {
  ...
  const { provider, apiKey, model, cardTitles, excludedTitles } = body as {
    provider: AIProvider
    apiKey: string
    model: string
  ...
  const prompt = buildFunFactPrompt(cardTitles || [], excludedTitles || [])
  ...
  const resp = await fetch(url, {
    ...
    body: JSON.stringify({
      model,
      messages: [
        { role: 'system', content: '你是一个知识科普专家，擅长发现反直觉的冷知识。请严格按照要求的JSON格式输出，不要包含markdown代码块标记。' },
