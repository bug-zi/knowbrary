...
import { buildResearchOutputPrompt } from '~/utils/ai-prompts'
...
export default defineEventHandler(async (event) => {
  ...
  const { provider, apiKey, model, topic, format, roadmap, notes, analysis } = body as {
    provider: AIProvider
    apiKey: string
    model: string
    topic: string
  ...
  if (!provider || !apiKey || !model || !topic || !format) {
  ...
  const prompt = buildResearchOutputPrompt(
    topic,
    format,
    roadmap || { items: [] },
    notes || [],
    analysis || { knowledgeMap: '', patterns: [], blindSpots: [] },
  )
  ...
  try {
    const output = parseJsonResponse(content)
    return { output }
  }
  catch {
